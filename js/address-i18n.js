...
jQuery( function( $ ) {
	...
	function facto_fe_field_is_required( field, is_required ) {
		if ( is_required ) {
		...
		.bind( 'country_to_state_changing', function( event, country, wrapper ) {
			...
			$.each( locale_fields, function( key, value ) {
				...
				if(key=='state') {
					...
					//field.find( 'label' ).html( field.find( 'label' ).html().replace("Región", "Comuna") );
				}
			});
		});
});